...
function start() {
	...
	if (hour == 20&& minute == 00&&(cm.获取当前星期() == 6 ||cm.获取当前星期() == 7 || cm.获取当前星期() == 1)) 
	{
	...
	var redm1 = getredm(6, 999);
	   ...
	   em.broadcastServerMsg(5120025, " 神秘人物出现在市场的 1 - 6 某个洞穴中，等待你的奇遇。", true);
                       }
                    }
    }
  if (hour == 20&& minute == 01 &&(cm.获取当前星期() == 6 ||cm.获取当前星期() == 7 || cm.获取当前星期() == 1)) 
	{
  ...
  if (hour == 19 && minute == 31 &&(cm.获取当前星期() == 6 ||cm.获取当前星期() == 7 || cm.获取当前星期() == 1)) 
	{
         map.removeNpc(9001106);
        em.broadcastServerMsg(5120025, " 神秘人物已经离开了。带着他的小宠物~", true);
