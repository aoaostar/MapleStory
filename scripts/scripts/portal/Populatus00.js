var pop = 2;
function enter(pi) {
    if (pi.getPlayer().getClient().getChannel() != 1 && pi.getPlayer().getClient().getChannel() != 2 && pi.getPlayer().getClient().getChannel() != 3 && pi.getPlayer().getClient().getChannel() != 4 && pi.getPlayer().getClient().getChannel() != 5 && pi.getPlayer().getClient().getChannel() != 6) {
        pi.playerMessage(5, "帕普拉图斯只能在頻道1-6频道能打。");
        return false;
    }
    ...
    if (!pi.haveItem(4031172)) {
		...
		pi.getPlayer().removeAll(4031870);
	}
