...
var sss ="#fUI/UIWindow.img/QuestIcon/3/0#";
	...
	if (status == 0) {
	    ...
	    var textz = "\r\n勇士:#d#h ##k，每次使用1500W冒险币可召唤扎昆本体和手臂。\r\n\r\n";

		textz += "#b#L0#解开封印【召唤--扎昆BOSS】#l\r\n\r\n";
	...
	if (selection == 0){
		...
		if (party == null || party.getLeader().getId() != cm.getPlayer().getId()) {
                ...
                cm.dispose();
			//	}else	if (cm.getClient().getPlayer().getBeans() < 3) {  
         //   cm.sendOk("修仙等级最高10级，只能初级修仙！");
              //  cm.dispose();
				}else if(cm.getLevel() < 70){
	            cm.sendOk("需要70级才能召唤.");
                cm.dispose();				
	// }	else   if (cm.getBossLog('PlayQuest151') >= 8) {
		//	cm.sendOk("你今天挑战次数超过8次!");
			//cm.dispose();			
	}	else if (cm.getMeso() < 15000000) {
			cm.sendOk("你身上不足1500万金币!");
			cm.dispose();
			}else{ 
		...
		//}else if (cm.getMeso() >= 5000000) {	//&&cm.haveItem(4001257,1)&&cm.haveItem(4001258,1)&&cm.haveItem(4001259,1)&&cm.haveItem(4001260,1)// #i04001257# #i04001258# #i04001259# #i04001260#
                        ...
                        cm.dispose();
						cm.喇叭(2,"[扎昆副本]：玩家" + cm.getPlayer().getName() + "开始挑战扎昆BOSS~");
...
if (party == null || party.getLeader().getId() != cm.getPlayer().getId()) {
	            ...
	            cm.sendOk("需要70级才能召唤.");
                cm.dispose();		
	}	else	if (cm.getBossLog('PlayQuest56') >= 1) {
