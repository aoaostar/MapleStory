...
/*    if (!cm.isLeader()) {
	...
	case 925100100:
	   ...
	   if (emp.equals("0")) {
		if (cm.haveItem(4001120,35)) {
			cm.给组队经验(500000);//给组队经验值
		    cm.sendNext("优秀！ 接下来我要#t4001121# #b35个#k!");
		    em.setProperty("stage2", "1");
		} else {
	   	    cm.sendNext("请给我35个#t4001120#!");
		}
	   } else if (emp.equals("1")) {
		if (cm.haveItem(4001121,35)) {
		    cm.sendNext("优秀！ 接下来我要#t4001122# #b35个#k!");
		    em.setProperty("stage2", "2");
			cm.给组队经验(500000);//给组队经验值
		} else {
	   	    cm.sendNext("请给我35个#t4001121#!");
		}
	...
	case 925100500:
	   if (cm.getMap().getAllMonstersThreadsafe().size() == 0) {
            cm.gainExp(1300000);
	//cm.gainItem(4031456, 1);//给枫叶珠
    cm.gainItem(4002000, 1);//邮票
	  cm.gainItem(4170000, 1);//蓝猪蛋
	  cm.gainItem(2340000, 2);//混沌卷
	  cm.gainItem(2000019, 5);//超级药水
