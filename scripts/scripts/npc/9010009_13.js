...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getPS() == 9){// cm.getPS()  的意思是 读取跑商值如果等于0 就得出他没有开始跑商 就运行他进行第一环跑商!
                txt += "#L1##b收集200个火独眼兽之尾#v4000007#交给我！#l";
                cm.sendSimple(txt);
            }else{
                txt += "你还没有完成之前的关卡，或者已经完成过了第十轮，继续进行下一环吧.!\r\n请第二天再来完成本环节！";
                cm.sendOk(txt);
                cm.dispose();
            }
            ...
            if (cm.haveItem(4000007,200) && cm.getLevel() <= 200 && cm.getLevel() >= 30){
				...
				cm.gainNX(1000);
                cm.gainExp(+100000);
                cm.sendOk("跑商第十轮完成!获得经验10万 金币20万 枫叶50个 点券1000 高等五彩水晶10个\r\n\r\n你已经完成过了然后了第十轮，继续进行下一环吧.");
                cm.dispose();
            }else{
                cm.sendOk("收集200个火独眼兽之尾#v4000007#交给我！");
                cm.dispose();
            }
        }
    }
