...
function getsidai(id){
    ...
    ps = con.prepareStatement("SELECT sidai FROM lingdi3 WHERE characterid = ?");
    ps.setInt(1, id);
...
function changesidai(sidai){
    var id = cm.getPlayer().getId();
	var con = DatabaseConnection.getConnection();
    ps = con.prepareStatement("update lingdi3 set sidai = ? where characterid = ?");
    ps.setInt(1, sidai);
    ps.setInt(2, id);
    ps.executeUpdate();
    ps.close();
...
function setsidai(sidai) {
    ...
    var ps = con1.prepareStatement("insert into lingdi3 (characterid, sidai) values (?,?)");
    ps.setInt(1, id);
    ps.setInt(2, sidai);
    ps.executeUpdate();
    ps.close();
