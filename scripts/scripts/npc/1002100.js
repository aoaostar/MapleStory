...
function action(mode, type, selection) {
    ...
    if (status == 0){
        ...
        for (var i = 0; i < items.length; i++)
            selStr += "\r\n#L" + i + "##i" + items[i][0] + "# (价格 : " + items[i][1] + " 枫币)#l";
        ...
        cm.sendYesNo("你购买这些 #r" + selection + "#k #b#t" + item[0] + "#(s)#k? #t" + item[0] + "# 费用为 " + item[1] + " 枫币 为一体，所以总出来是 #r" + (item[1] * selection) + "#k 枫币.");
        amount = selection;
    } else if (status == 3) {
        if (cm.getMeso() < item[1] * amount)
            cm.sendNext("确认你的枫币是否足够,和检查你的消耗拦是否足够,如果有你至少 #r" + (item[1] * selectedItem) + "#k 枫币.");
        else {
            if (cm.canHold(item[0])) {
                ...
                cm.sendNext("确认你的枫币是否足够,和检查你的消耗拦是否足够..");
        }
        cm.dispose();
    } 
	*/
