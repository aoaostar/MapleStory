...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status >= 0 && mode == 0) {
            ...
            return;
      //  }
		//	if (cm.getHour() < 22 || cm.getHour() > 23 ){//hour=时//month=月//
	//cm.sendOk("游山玩水活动时间为；#r22 ：00 - 23 ：00");
    // cm.dispose();
	 }
        if (mode == 1)
            status++;
        ...
        if (status == 0) {
            ...
            cm.sendSimple("#d把你的血控制在以下值，就可以突破极限。\r\n\r\n\r\n"+a1+"");
			...
			if (cm.getPlayerStat("HP") == 250 && cm.getBossLog("mrkx") < 5){
				...
				cm.sendOk("血量控制已经掌握，请小退游戏进行刷新 + 1 5 HP。");
				cm.worldMessage(6,"[每日控血]：恭喜，玩家 "+cm.getName()+" 真厉害，成功控制自己的血量，增强了体质 HP + 15 。");
				cm.dispose();
			} else {
				cm.sendOk("你还无法控制你的血。每天可以提升5次。");
				cm.dispose();
				return;
			}
	    	}
