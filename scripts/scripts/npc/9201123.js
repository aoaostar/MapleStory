...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
	    var a1 = "#L1##b" + 正方箭头 + "装备力量提升#i4000038# x 1     金币 x 100.000\r\n";
		...
		if (cm.haveItem(4000038, 1) && cm.getMeso()>=100000 ) {
			...
			if(rand<30){
			cm.gainEquiPproperty(0,0,0,3,0,0,0,15,0,0,0);//次数/w攻击/m攻击/力量/敏捷/智力/运气/HP/MP/命中/回避/
			cm.sendOk("#fEffect/BasicEff.img/Fishing/6#\r\n\r\n#r增加属性#k;#b   力 量 + 3  生 命 + 15");
			cm.dispose();
			cm.worldMessage(6,"[公告]：恭喜，玩家  "+cm.getName()+"  装备提升力量成功。");
			return;
			}
			else {
			cm.gainEquiPproperty(0,0,0,0,-3,0,0,-15,0,0,0);
			//cm.sendOk("真遗憾，提升失败，还损失了一些属性。");
			cm.dispose();
			cm.sendOk("很遗憾失败了:#r\r\n力量-3 \r\n生命-15");
			cm.worldMessage(6,"[公告]：遗憾，玩家  "+cm.getName()+"  装备提升力量失败。");
			return;
				}
		} else {
