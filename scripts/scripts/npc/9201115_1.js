...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == 180000001) {
        ...
        selStr += "#r您是否用#i4000000# 继续添加？？";
        cm.sendYesNo(selStr);
    }else if(status == 1){
		if(!cm.haveItem(item)){
			cm.sendOk("你身上没有#i4000000# 哦！");
			cm.dispose();
			return;
		}
...
function getsidai(id){
    ...
    ps = con.prepareStatement("SELECT sidai FROM lingdi1 WHERE characterid = ?");
    ps.setInt(1, id);
...
function changesidai(sidai){
    var id = cm.getPlayer().getId();
	var con = DatabaseConnection.getConnection();
    ps = con.prepareStatement("update lingdi1 set sidai = ? where characterid = ?");
    ps.setInt(1, sidai);
    ps.setInt(2, id);
    ps.executeUpdate();
    ps.close();
...
function setsidai(sidai) {
    ...
    var ps = con1.prepareStatement("insert into lingdi1 (characterid, sidai) values (?,?)");
    ps.setInt(1, id);
    ps.setInt(2, sidai);
    ps.executeUpdate();
    ps.close();
