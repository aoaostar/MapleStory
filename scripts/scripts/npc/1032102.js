...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("你好！我是玛丽，我可以复活或者升级你的宠物哦~ #b\r\n#L0#进化我的宠物龙.#l\r\n#L1#进化我的机器人.#l\r\n#L2#救救我的宠物.#l#k");
		} else if (status == 1) {
			if (selection == 0) {
				var currentpet = null;
				for (var i = 0; i < 3; i++) {
				...
				if (pet == null || !cm.haveItem(5380000,1)) {
					...
					cm.sendOk("你的宠物龙进化啦，它曾经是 #i" + id + "##t" + id + "#, 现在进化成 #i" + after + "##t" + after + "#啦!");
					cm.dispose();
				}
			} else if (selection == 1) {
				var currentpet = null;
				...
				if (pet == null || !cm.haveItem(5380000,1)) {
					cm.sendOk("我可能无法帮助你！ 你必须给我 #i5380000##t5380000#,如果你的宠物是以下之一： #g#i5000048##t5000048##k, #r#i5000049##t5000049##k, #b#i5000050##t5000050##k, #d#i5000051##t5000051##k, #d#i5000052##t5000052##k, 或者 #e#i5000053##t5000053##n 只要等级达到15级，我就可以帮你升级哟.");
					cm.dispose();
				} else {
					var id = pet.getPetItemId();
					var name = pet.getName();
					...
					cm.gainPet(after, name, level, closeness, fullness, 45, flag);
					cm.getPlayer().spawnPet(slot);
					cm.sendOk("你的机器人成功升级啦!!它曾经是 #i" + id + "##t" + id + "#, 现在变成了 #i" + after + "##t" + after + "#!");
					cm.dispose();
				}
