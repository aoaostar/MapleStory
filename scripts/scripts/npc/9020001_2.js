...
function boxStage(cm) {
    ...
    if (cm.isLeader()) { // leader
        if (status == 0) {
            ...
            if (preamble == null) {
                cm.sendNext("你好。欢迎来到第2阶段，在我旁边，有4个藤蔓。需要2个队友爬在藤蔓上，队长在来点击我，看是否正确。祝你好运！");
		    ...
		    //cm.warpParty(910340300);
                    cm.sendNext("恭喜你！ 你成功通过了第2阶段！快点，向第3阶段前进吧！");
                } else {
                    var totplayers = 0;
                    ...
                    if (totplayers == 2 || debug) {
                        ...
                        if (debug) {
                            ...
                            cm.sendNext("需要 2名队员爬在藤蔓上 组成不同的组合. 只允许 2名队员爬在藤蔓上, 请记住你们已经使用过的错误组合,方便快速通关.");
                            cm.dispose();
                        }
                    }
                }
