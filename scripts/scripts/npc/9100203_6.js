...
function action(mode, type, selection) {
    if (mode == -1) {
		...
		if(cm.getBossLog("5") == 0 ){
    		cm.sendOk("你第五个号码还没摇呢。");
    		cm.dispose();
    		return;
			 }
		
    	if(cm.getBossLog("6") > 0){
    		cm.sendOk("第六个号码已经摇到了。");
    		cm.dispose();
    		return;
			 }
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendAcceptDecline("这里是第六个号码摇号。");
        } else if (status == 1) {
           ...
           // } else if (cm.getText() < 100) {
               ...
               // cm.dispose();
                 ...
                 if (chance <= 1) { 
						...
						cm.setBossLog("6");
						cm.setBossLog("zymxd");
						cm.setBossLog("yaoyiyao");
						cm.setBpLog("yaoyiyao");
