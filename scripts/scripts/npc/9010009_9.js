    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getPS() == 5){// cm.getPS()  的意思是 读取跑商值如果等于0 就得出他没有开始跑商 就运行他进行第一环跑商!
                txt += "#L1##b收集100个鸭蛋#v4000004#交给我！#l";
                cm.sendSimple(txt);
            }else{
            ...
            if (cm.haveItem(4000188,100) && cm.getLevel() <= 100 && cm.getLevel() >= 30){
                ...
                cm.gainMeso(200000)
                cm.gainExp(+100000);
                cm.sendOk("跑商第六轮完成!经验10万 金币20万 枫叶50个\n\r\n你已经完成过了然后了第六轮，继续进行下一环吧.");
                cm.dispose();
            }else{
                cm.sendOk("收集100个鸭蛋#v4000004#交给我!");
                cm.dispose();
            }
        }
    }
