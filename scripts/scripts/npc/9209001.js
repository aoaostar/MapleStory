...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (sel == undefined)
            sel = selection;
        if (selection == 0) {
            cm.sendNext("好了，我将送你到冒险市场地图.");
        } else
            cm.sendSimple("请问您想了解冒险市场哪部分??#b\r\n#L0#那是什么地方??\r\n#L1#能在冒险市场干什么事情??\r\n#L2#我没有任何问题");
    } else if(status == 1) {
        if (sel == 0) {
            ...
            cm.sendNext("冒险市场只有在假日开放。如果您在其他城镇发现我也可以在那找到我进入, 我几乎无所不在!!!");
            status -= 2;
        } else if (selection == 1)
            cm.sendSimple("你可以在冒险市场找到其他地方很难找到的罕见商品.#b\r\n#L0#购买特殊物品\r\n#L1#帮助家禽农场业主");
        else {
        ...
        if (sel2 == undefined)
            sel2 = selection;
        if (sel2 == 0)
            cm.sendNext("你可以找到许多道具在冒险市场,价格很容易有变动,所以你最好在他改变价格前往采买,因为他们变动的时候很便宜的!!");
        else
            cm.sendNext("除了商人你还可以找到养鸡场煮人在冒险市场里面,帮助咪咪和她孵化的蛋,直到鸡长大变成一只好吃的鸡!");
    } else if (status == 3) {
        if (sel2 == 0)
        ...
        if (sel2 == 0)
            cm.sendNextPrev("通过购买善于在冒险市场价格出售给商家的中介时，其价值上升测试你的业务的机智！");
        else
