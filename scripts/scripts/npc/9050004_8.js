    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getPS() == 4){// cm.getPS()  的意思是 读取跑商值如果等于0 就得出他没有开始跑商 就运行他进行第一环跑商!
                txt += "#L1##b收集300个半人马的骨头#v4000234#交给我！#l";
                cm.sendSimple(txt);
            }else{
            ...
            if (cm.haveItem(4000234,300) && cm.getLevel() >= 81 && cm.getLevel() <= 150){
                cm.gainPS(1);//cm.gainPS(1);  的意思是 你完成跑商第一环的时候给予你 跑商值+1这样你就无法在重复做第一环了。只有凌晨12点刷新才行！
		
               cm.gainItem(2340000, 1);
			   cm.gainItem(4000234, -300);
                cm.gainExp(+280000);
                cm.sendOk("跑商第五轮完成!获得祝福卷轴一张、经验=280000\r\n\r\n你已经完成过了然后了第五轮，继续进行下一环吧.");
                cm.dispose();
            }else{
                cm.sendOk("收集300个半人马的骨头#v4000234#交给我!");
                cm.dispose();
            }
        }
    }
