...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.askAcceptDecline("训练进行得怎么样了？哇哦~！看着你，我可以看出你的能力已经今非昔比了，太让人吃惊了！我知道你很忙战神。但无论如何，请你回里恩来找我。");
    } else if (status == 1) {
        qm.forceStartQuest(21200, "3"); //??
        qm.forceCompleteQuest();
        qm.forceStartQuest(21202); //skip just in case
        qm.forceStartQuest(21203, "0");
        qm.sendOk("Your #b巨大的战斧#k that's being kept in #bRien#k is acting strange all of a sudden. According to the book, the pole arm reacts like this when it's calling for its master. #bMaybe it's calling for you?#k? 请回到岛上找出答案。");
        qm.dispose();
    }
}
